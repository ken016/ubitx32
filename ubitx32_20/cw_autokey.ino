...
void sendCWChar(char cwKeyChar)
{
  ...
  if (cwKeyChar == '>' || cwKeyChar == '<')
    ...
    for (uint16_t i = callsignStartIndex; i <= callsignEndIndex; i++)
    {
      ...
      if (i < callsignEndIndex) delay_background(conf.cwSpeed * 3, 4); //
    }
  ...
  {
    charLength = 0;
    delay_background(conf.cwSpeed * 4, 4); //7 -> basic interval is 3
  }
  ...
  {
    cwKeydown();
    delay_background(sendBuff[i] == 0?conf.cwSpeed:conf.cwSpeed * 3, 4);
    cwKeyUp();
    if (i != charLength -1)
      delay_background(conf.cwSpeed, 4);
  }
}
...
void controlAutoCW(){
    ...
    if (isCWAutoMode == 1) {                                    //ready status
      if (digitalRead(PTT) == LOW)                              //PTT Down : Start Auto CW or DialMode Change
      {
        ...
        else if (autoCWbeforeTime < millis())                   //while press PTT, OK Long push then Send Auto CW Text
        {
          ...
          if (!inTx){                                           //if not TX Status, change RX -> TX
            ...
            delay_background(conf.delayBeforeCWStartTime * 2, 2);    //for External AMP or personal situation
          }
        }
      }
      else if (pttBeforeStatus == 0 && autoCWbeforeTime > 0)    //while reade status LOW -> HIGH (before Auto send Before)
      {
          ...
          if (autoCWbeforeTime > millis())                      //short Press -> ? DialModeChange
          {
            ...
            if (cwAutoDialType == 0)
              printLineF(1,F("Dial:Select Text"));
            else
              printLineF(1,F("Dial:Freq Tune"));
    ...
    if (isCWAutoMode == 2) {                                    //Sending Mode
        ...
        if (isAutoCWHold == 0 && (millis() - autoCWbeforeTime > conf.cwSpeed * 3))
        {
          if (!inTx){                                           //if not TX Status, change RX -> TX
            keyDown = 0;
            startTx(TX_CW, 0);  //disable updateDisplay Command for reduce latency time
          ...
          if (autoCWSendCharIndex > autoCWSendCharEndIndex) {          //finish auto cw send
            ...
            else
            {
              ...
              delay_background(conf.cwDelayTime * 10, 2);
              stopTx();
            }
          }
